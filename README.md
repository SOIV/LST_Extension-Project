...
YouTube 중심 커뮤니티 자막 플랫폼 + Chrome 확장 프로그램 + Desktop 보조 앱 프로젝트
...
추가로 **Lite Helper / Desktop App**을 통해 브라우저 단독으로 처리하기 어려운 STT, 오디오 캡처, 고급 AI 번역 기능을 분리하는 방향으로 설계하고 있습니다.
...
| Chrome 확장 프로그램 | 개발 버전 | YouTube 커뮤니티 자막 로드와 자체 오버레이 렌더러 중심 |
| 실시간 STT | 설계/계획 | Extension 단독 Web Speech/API STT와 Desktop 보조 STT를 분리 |
| Lite Helper | 계획 | 브라우저 확장의 한계를 보완하는 경량 Desktop 보조 앱 |
| Full Desktop App | 계획 | 고급 STT, 오디오 캡처, AI 번역, 로컬 LLM/Ollama 연동 후보 |
| 공개 Docs 사이트 | 계획 | 사용자/개발자용 문서 사이트는 아직 생성 전 |
...
- 팝업 UI와 플레이어 패널 UI
- 한국어, 영어, 일본어 로케일
- 향후 실시간 STT와 번역 자막 기능 확장 예정

### STT와 번역 방향

LST의 STT/번역 기능은 실행 위치를 명확히 분리하는 방향입니다. 아래 표는 현재 릴리스 상태가 아니라 기능 배치 방향을 의미합니다.
