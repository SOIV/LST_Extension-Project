...
현재 자막 렌더러가 HTML/DOM 기반이므로 **별도 엔진 교체 없이** 브라우저 네이티브 `<ruby>` 태그를 사용할 수 있다.

```html
<ruby>日本語<rt>にほんご</rt></ruby>
```

- 렌더러에서 텍스트를 `innerHTML`로 삽입하도록 변경 필요
- CSS에 `ruby`, `rt` 스타일(크기, 색상 등) 추가 필요
- 외부 입력을 그대로 `innerHTML`에 넣는 구조는 XSS 위험이 있으므로, 후리가나 변환은 내부 파이프라인에서만 수행하고 최종 마크업 생성 시 화이트리스트 방식으로 처리

#### 읽기 정보 생성 (형태소 분석)

STT 결과는 평문이라 어떤 한자를 어떻게 읽는지 별도로 계산해야 한다.

| 방식 | 도구 | 처리 위치 | 비고 |
|------|------|-----------|------|
| 형태소 분석 | `kuroshiro` + `kuromoji.js` | 클라이언트 | 사전 파일 ~15MB, 초기 로딩 후 오프라인 동작 가능 |
| 형태소 분석 | `kuromoji.js` 단독 | 클라이언트 | 경량, 정확도 다소 낮음 |
| 외부 API | Google NL API 등 | 서버 사이드 | 정확도 최고, 레이턴시 및 비용 발생 |

STT 실시간 특성상 **`kuroshiro` + `kuromoji.js` 클라이언트 조합**이 현실적인 1차 구현 방향이다.
...
STT 평문
 ↓
kuromoji 형태소 분석 → 각 토큰에 reading(읽기) 부여
 ↓
<ruby>한자<rt>읽기</rt></ruby> 마크업 생성
 ↓
자막 렌더러 (innerHTML 삽입)
```
