...
## ✅ 17. 외부 사이트(자막 없는 콘텐츠) 실시간 STT 뷰어 — 서브/베타 기능

### 배경
- MORT류 도구는 OCR(화면 내 텍스트 추출) 기반 → 이미 존재하는 텍스트에 의존
- LST의 본질은 사운드/STT 기반 → 자막이 아예 없는 콘텐츠에도 새로 자막 생성 가능
- 이 차별점을 살려, 일본 애니메이션/해외 드라마처럼 자막이 없는 사이트(주로 국내 OTT/방영 사이트)에서
  탭 오디오 캡처 → 실시간 STT → 번역 → 오버레이로 보여주는 기능을 구상

### 포지셔닝
- **메인 기능 아님**: 메인은 어디까지나 유튜브 실시간 STT + 유튜브 커뮤니티 자막 플랫폼
- 이 기능은 **서브/베타** 기능으로 취급

### 운영 분리 방침 (저작권 민감성 때문)
대상 사이트 대부분이 저작권 보호 조치가 걸려 있는 상업 플랫폼이라, 메인 커뮤니티 자막 플랫폼과 완전히 같은 시스템으로 묶지 않는다.

- **로그인/계정**: 통합 — 기존 Supabase Auth/OAuth 그대로 공유
- **데이터(DB)**: 분리 — 베타 기능 전용 테이블/스키마로 메인 커뮤니티 자막 테이블과 분리
- **저장소(파일)**: 처음부터 분리 — 별도 Cloudflare R2 버킷(필요 시 별도 계정 단위까지 고려)

**Why:** 저작권 문제로 특정 콘텐츠를 강제 삭제/차단해야 하는 상황이 생겼을 때, 메인 서비스(유튜브 대상, 정상 운영)까지 리스크가 전이되지 않도록 인프라 단위로 격리하기 위함. DB/저장소가 분리돼 있으면 베타 쪽만 빠르게 끊어내거나 통째로 내릴 수 있다.

- 커뮤니티 자막 플랫폼의 업로드/검수/신뢰도 시스템을 이쪽에 그대로 붙이지 않는 방향도 고려 (개인용 STT 뷰어 성격에 가깝게 가벼운 운영)
- 별도 페이지/서브도메인으로 구성

### 조건부 확장 (베타가 정상 운영된 이후)
- AniList / Annict / anitabi 같은 애니메이션 작품 DB API 연동
- 작품 메타데이터(제목, 에피소드 정보, 이미지 등)를 불러와 뷰어 페이지에 표시
- 전제 조건: 위 베타 기능 자체가 안정적으로 운영된 뒤 검토 (지금 단계의 확정 스코프 아님)

### 서브 아이디어: 기존 작업 자막 재생 (STT 대신 재생 시간 동기화)
- 넷플릭스/일본 OTT 같은 상업 플랫폼에서도 `<video>` 엘리먼트의 `currentTime`은 content script로 읽을 수 있음 (Language Reactor 등 기존 확장이 이미 쓰는 방식, 기술적으로 어렵지 않음)
- 이 값을 기준으로 STT를 새로 돌리는 대신, **이미 작업(제작)된 자막 파일을 시간 동기화해서 재생**하는 것도 가능
- 관건은 STT가 아니라 **콘텐츠 식별/매칭**: 넷플릭스는 `watch/{id}` URL을 키로 활용 가능하지만, 일본 OTT는 사이트마다 URL 구조가 달라 사이트별 어댑터 필요
- 자막 표시(오버레이 렌더러)는 기존 컴포넌트 재사용 가능 → 신규 구현 범위는 "시간 읽기 + 콘텐츠 매칭 + 자막 파일 로드"로 한정됨
- 저작권 리스크는 위 STT 뷰어와 동일 카테고리(오히려 완성된 자막 파일을 다루는 만큼 더 민감할 수 있음) → 위에서 정한 로그인 통합/DB·스토리지 분리 운영 방침 그대로 적용
